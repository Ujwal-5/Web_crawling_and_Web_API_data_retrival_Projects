...
class DbService:
    ...
    def update_the_record(self, status, table, column, status_column, keyword, isvalid, numeroNif, nomeContribuinte):
        cursor = self.connection.cursor()
        query = f"UPDATE {table} SET {status_column} = {status}, IS_VALID ={isvalid}, numeroNif = '''{numeroNif}''', nomeContribuinte = '''{nomeContribuinte}''' WHERE {column}= '{keyword}'"
        print(query)
