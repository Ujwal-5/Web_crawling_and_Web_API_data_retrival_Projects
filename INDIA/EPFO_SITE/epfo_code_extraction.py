...
while True:
    try:
        ...
        class DbService:
            ...
            def insert_the_record(self, list_I, keyword, max_retries=3, retry_delay=1):
                cursor = None
                try:
                    cursor = self.connection.cursor(prepared=True)
                    params = list_I
                    print(params)
                    # If the number of records is greater than 1000, insert in batches of 100
                    batch_size = 50
                    ...
                    for i in range(0, total_records, batch_size):
                        ...
                        insert_query = "INSERT IGNORE INTO URL_XSINEPFO_V2 (id, establishment_code) VALUES "
                        
                        # # Construct the values part of the query for the batch
                        values_part = ', '.join([str(param) for param in batch_params])
                        print(values_part)

                        # # Combine the query and values for the batch
                        insert_query += values_part
                        print(insert_query)
                    ...
                    if max_retries > 0:
                        print("Retrying in a moment...")
                        time.sleep(retry_delay)
                        return self.insert_the_record(list_I, keyword, max_retries=max_retries-1, retry_delay=retry_delay)
                    else:
        ...
        response_search = requests.request("POST", search_url, headers=search_headers, data=search_payload, timeout=20)
        ...
        print(data_list)
        database_service.insert_the_record(data_list, keyword)
    except: pass
